...
export const ThreeStepLogin: React.FC<ThreeStepLoginProps> = ({
  ...
  const [quickDemoLoadingRole, setQuickDemoLoadingRole] = useState<UserRole | null>(null);

  // 1-Click Quick Demo Login for the specified or currently active role
  const handleQuickDemoLogin = async (roleToLogin?: UserRole) => {
    const targetRole = roleToLogin || selectedRole;
    setQuickDemoLoadingRole(targetRole);
    setError(null);
    try {
      const user = await demoLogin(targetRole);
      setSuccessMessage(`Logged in instantly as Demo ${targetRole.toUpperCase()}: ${user.full_name}`);
      if (onSuccess) {
        onSuccess();
      }
    } catch (err: any) {
      setError(err.message || `Failed to authenticate quick demo login for ${targetRole}.`);
    } finally {
      setQuickDemoLoadingRole(null);
    }
  };
      ...
      <div className="p-5 sm:p-7">
                  ...
                  >
                    ...
                    <div className="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleQuickDemoLogin(item.role);
                        }}
                        disabled={quickDemoLoadingRole !== null}
                        className="px-2.5 py-1 rounded-lg bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/50 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 font-bold text-[11px] transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                      >
                        {quickDemoLoadingRole === item.role ? (
        ...
        {currentStep === 1 && (
          <form onSubmit={handleStep1Submit} className="space-y-4">
            ...
            {/* Quick Demo Login Helper for Each Role */}
            <div className="p-3.5 rounded-2xl bg-gradient-to-r from-blue-50/80 via-indigo-50/50 to-slate-50 dark:from-blue-950/30 dark:via-indigo-950/20 dark:to-slate-900/40 border border-blue-200/80 dark:border-blue-800/50 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-blue-600 text-white shadow-xs">
                    {selectedRole}
                  </span>
                  <span className="text-xs font-bold text-slate-800 dark:text-slate-200">
                    {DEMO_PRESETS[selectedRole].name}
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Preset: <span className="font-mono text-slate-700 dark:text-slate-300 font-semibold">{DEMO_PRESETS[selectedRole].email}</span>
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 active:bg-blue-800 disabled:opacity-60 text-white font-bold text-xs shadow-sm shadow-blue-500/20 transition-all flex items-center gap-1.5 cursor-pointer"
                >
                  {quickDemoLoadingRole === selectedRole ? (
                    <>
                ...
                <button
                  ...
                  title="AutoFill credentials into fields to test manual verification"
                  onClick={() => {
                    const p = DEMO_PRESETS[selectedRole];
                    setEmail(p.email);
                    setPassword(p.pass);
                  }}
                  className="px-2.5 py-1.5 rounded-xl bg-white hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200/90 dark:border-slate-700 text-xs font-semibold shadow-2xs transition-colors cursor-pointer"
                >
                  AutoFill
                </button>
              </div>
